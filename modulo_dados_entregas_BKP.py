...
class BancoEntregas(ctk.CTkFrame):
    ...
    def fechar_ciclo_diario(self):
        ...
        try:
            ...
            # Localização dos arquivos (padrão SISADAM)
            p_base = pathlib.Path.home() / "Documents" / "SISADAM"
            p_banco = p_base / "Arquivos_Banco" / "dados_entregas.db"
            p_cofre = p_base / "Cofre_Backup" / "Entregas"
            p_cofre.mkdir(parents=True, exist_ok=True)
            
            # Define a data do backup (referente ao dia que passou)
            ...
            # Limpa a Treeview e os campos para o novo turno/dia
            self.carregar_dados()
            self.nova_entrada()
    ...
    # --- FUNÇÕES DE SUPORTE (MOVIDAS PARA CIMA PARA EVITAR ERRO DE ATRIBUTO) ---
    ...
    def buscar_moradores_unidade(self, *args):
        try:
            ...
            rows = conn.execute("SELECT nome_morador, ddi, ddd, telefone FROM moradores WHERE bloco=? AND apartamento=?", (bloco, apto)).fetchall()
            conn.close()
            self.moradores_atuais = {}
            ...
            if rows:
                for r in rows:
                    nome = r[0].upper()
                    self.moradores_atuais[nome] = f"{r[1]}{r[2]}{r[3]}".replace(" ", "").replace("-", "")
                    nomes.append(nome)
    ...
    def montar_interface(self):
        ...
        self.btn_wpp = ctk.CTkButton(self.f_acoes, text="🟢 WPP", width=80, fg_color="#25D366", command=self.preparar_whatsapp).pack(side="left", padx=2)
        ...
        self.f_status_wpp = ctk.CTkFrame(self, fg_color="transparent")
        self.f_status_wpp.pack(fill="x", padx=10)
        self.progresso_wpp = ctk.CTkProgressBar(self.f_status_wpp, width=300, height=8); self.progresso_wpp.set(0); self.progresso_wpp.pack(side="left", padx=5)
        self.lbl_status_wpp = ctk.CTkLabel(self.f_status_wpp, text="AGUARDANDO", font=("Arial", 9), text_color="gray"); self.lbl_status_wpp.pack(side="left")
    ...
    def preparar_whatsapp(self):
        threading.Thread(target=self.rotina_wpp, daemon=True).start()

    def rotina_wpp(self):
        tel = self.moradores_atuais.get(self.cb_destinatario.get())
        if not tel: return
        self.lbl_status_wpp.configure(text="⚙️ COPIANDO FOTO...", text_color="#2196F3"); self.progresso_wpp.set(0.4)
        if self.foto_pacote:
            cmd = f"Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Clipboard]::SetImage([System.Drawing.Image]::FromFile('{os.path.abspath(self.foto_pacote)}'))"
            subprocess.run(["powershell", "-command", cmd], creationflags=0x08000000)
        self.progresso_wpp.set(1.0); self.lbl_status_wpp.configure(text="✅ PRONTO!", text_color="#00E676")
        import webbrowser
        webbrowser.open(f"https://web.whatsapp.com/send?phone={tel}&text={urllib.parse.quote('Encomenda da ' + self.ent_empresa.get().upper() + ' na portaria.')}")
    ...
    def preparar_edicao(self):
        ...
        conn = sqlite3.connect(str(ARQUIVO_DB_ENTREGAS))
        # Seleção blindada: empresa, entregador, bloco, apartamento, tipo, status, foto_ent, foto_pac, destinatário
