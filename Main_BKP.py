...
class JanelaGestaoUsuarios(ctk.CTkToplevel):
    def __init__(self, master, nivel_acesso):
        ...
        self.en_u = ctk.CTkEntry(self.f_cad, placeholder_text="Novo Usuário"); self.en_u.pack(pady=5, padx=10)
        self.en_p = ctk.CTkEntry(self.f_cad, placeholder_text="Senha", show="*"); self.en_p.pack(pady=5, padx=10)
        ...
        if self.nivel != "MASTER":
            for child in self.f_cad.winfo_children():
...
class JanelaLogin(ctk.CTkToplevel):
    def __init__(self, master, perfil):
        ...
        self.ent_user = ctk.CTkEntry(self, placeholder_text="Usuário", width=250); self.ent_user.pack(pady=10)
        self.ent_pass = ctk.CTkEntry(self, placeholder_text="Senha", show="*", width=250); self.ent_pass.pack(pady=10)
...
class AppSISADAM(ctk.CTk):
    def __init__(self):
        ...
        self.lista_imagens = ["FRENTE-NOVA.png", "FRENTE-02.png", "FRENTE-03.png"]
        self.indice_slide = 0
    ...
    def rodar_slideshow(self):
        try:
            largura = self.frame_main.winfo_width()
            altura = self.frame_main.winfo_height()
            if largura < 10: largura, altura = 900, 750
            img_path = self.lista_imagens[self.indice_slide]
    ...
    def abrir_moradores(self):
        ...
        try:
            ...
            tela = modulo_dados_moradores.BancoMoradores(self.frame_main, nivel=self.sessao_ativa)
    ...
    def abrir_veiculos_visitantes(self):
        ...
        try:
            ...
            self.tela_atual = modulo_veiculos_visitantes.BancoVeiculosVisitantes(self.frame_main, nivel=self.sessao_ativa)
            self.tela_atual.place(relx=0, rely=0, relwidth=1, relheight=1)
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao carregar Veículos Visitantes: {e}")

    def abrir_entregas(self):
        self.limpar_tela_principal()
        try:
            import modulo_dados_entregas
            tela = modulo_dados_entregas.BancoEntregas(self.frame_main, nivel=self.sessao_ativa)
            tela.place(relx=0, rely=0, relwidth=1, relheight=1)
        except: pass
    ...
    def abrir_prestadores(self):
        self.limpar_tela_principal()
        try:
            import modulo_dados_prestadores_BPK
            self.tela_atual = modulo_dados_prestadores_BPK.BancoPrestadores(self.frame_main, nivel=self.sessao_ativa)
            self.tela_atual.place(relx=0, rely=0, relwidth=1, relheight=1)
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao carregar Prestadores: {e}")
